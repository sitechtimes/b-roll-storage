...
const processImage = (imagePath) => {
    return new Promise((resolve, reject) => {
        ...
        const pythonExe = path.join(__dirname, "../.venv", isWindows ? "Scripts" : "bin", isWindows ? "python.exe" : "python");
...
async function processVideo(videoPath) {
    ...
    async function extractFrameAt(timeSec) {
        const out = tmpFramePath();
        return new Promise((resolve, reject) => {
            // determine ffmpeg binary: env > ffmpeg-static > 'ffmpeg'
            let ffmpegCmd = process.env.FFMPEG_PATH;
            if (!ffmpegCmd) {
