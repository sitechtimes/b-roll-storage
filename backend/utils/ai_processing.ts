...
const processImage = (imagePath: string) => {
  return new Promise((resolve, reject) => {
    ...
    const scriptPath = path.resolve(
      __dirname,
      "../recognize-anything/inference_ram_plus.py",
    );
    const modelPath = path.resolve(
      __dirname,
      "../recognize-anything/pretrained/ram_plus_swin_large_14m.pth",
    );
    ...
    pythonProcess.stdout.on(
      "data",
      (data: { toString: () => string }) => (output += data.toString()),
    );
    pythonProcess.stderr.on(
      "data",
      (data: { toString: () => string }) => (errorData += data.toString()),
    );

    pythonProcess.on("close", (code: number) => {
      if (code !== 0) {
        return reject(
          new Error(`Python script failed (code ${code}): ${errorData}`),
        );
      }
      ...
      const tagLine = lines.find((l) =>
        l.toLowerCase().startsWith("image tags:"),
      );
      if (tagLine) {
...
async function processVideo(videoPath: string): Promise<string[]> {
  ...
  function getDuration(vpath: string): number | null {
    try {
      ...
      try {
        ...
        const ff = execFileSync("ffmpeg", ["-i", videoPath], {
          encoding: "utf8",
  ...
  function tmpFramePath(): string {
    return path.join(
      os.tmpdir(),
      `frame-${Date.now()}-${crypto.randomBytes(4).toString("hex")}.jpg`,
    );
  }
  ...
  async function extractFrameAt(timeSec: number): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!ffmpegCmd) {
        console.error("FFMPEG IS NOT INSTALLED! DID YOU ADD IT TO THE PATH?");
      }
